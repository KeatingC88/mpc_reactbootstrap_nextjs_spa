...
export const Register_End_User_Twitch_Account = () => async (dispatch: AppDispatch, getState: () => Current_Redux_State) => {
    ...
    if (code) {
        axios.post(`${USERS_SERVER_ADDRESS}/Twitch/Register`, {
            code: code,
            ...
            return await new Promise(async (resolve) => {
                ...
                let jwt_data = JWT_Decoder(mpc_data.token)
                ...
                if (!jwt_client_issuer_match) {
                    ...
                    await dispatch({
                        type: UPDATE_END_USER_ACCOUNT_STATE, payload: {
                            account_type: await parseInt(Decrypt(`${jwt_data[fetch_key[0]]}`)) === await parseInt(mpc_data.account_type) ? parseInt(mpc_data.account_type) : null,
                            created_on: await parseInt(mpc_data.created_on),
                            email_address: await Decrypt(`${jwt_data[fetch_key[5]]}`) === twitch_data.email ? twitch_data.email : null,
                            id: (await parseInt(Decrypt(`${jwt_data[fetch_key[1]]}`)) === await parseInt(mpc_data.id)) ? parseInt(mpc_data.id) : null,
                            login_on: await parseInt(mpc_data.login_on),
                            token: mpc_data.token,
                            name: twitch_data.display_name + mpc_data.name,
                            roles: (await Decrypt(`${jwt_data[fetch_key[2]]}`) === mpc_data.roles.slice(1, -1)) ? mpc_data.roles.slice(1, -1) : null,
                            client_address: (await Decrypt(`${jwt_data[fetch_key[4]]}`) === CLIENT_ADDRESS) ? CLIENT_ADDRESS : null,
                            groups: (await Decrypt(`${jwt_data[fetch_key[3]]}`) === mpc_data.groups.slice(1, -1)) ? mpc_data.groups.slice(1, -1) : null,
