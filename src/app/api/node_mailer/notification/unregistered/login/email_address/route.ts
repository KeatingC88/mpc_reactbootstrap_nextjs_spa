...
export const POST = async (req: NextRequest) => {
    try {
        ...
        await axios.post(`${MAIL_SERVER_ADDRESS}/Send/Notification/Unregistered/Email`, {
        ...
        let response = await axios.post(`${MAIL_SERVER_ADDRESS}/Send/Notification/Unregistered/Email`, {
            email_address: Encrypt(`${dto.email_address}`),
            language: Encrypt(`${dto.language}`),
            region: Encrypt(`${dto.region}`),
            local_time: Encrypt(`${dto.local_time}`),
