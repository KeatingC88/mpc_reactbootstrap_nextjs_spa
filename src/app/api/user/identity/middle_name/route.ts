...
export const PUT = async (req: NextRequest) => {
    try {
        ...
        if (token) {
            ...
            await axios.put(`${USERS_SERVER_ADDRESS}/Identity/MiddleName`, {
                middle_name: Encrypt(`${dto.middle_name}`),
                id: Encrypt(`${dto.id}`),
                token: token,
                account_type: Encrypt(`${dto.account_type}`),
                login_type: Encrypt(`${dto.login_type}`),
                language: Encrypt(`${dto.language.split(`-`)[0]}`),
                region: Encrypt(`${dto.language.split(`-`)[1]}`),
                ...
                await axios.post(`${USERS_PROFILE_CACHE_SERVER_ADDRESS}/set/user/profile`, {
                    ...
                    birth_date: await Encrypt(`${dto.birth_date}`),
                    ethnicity: await Encrypt(`${dto.ethnicity}`),
                    first_name: await Encrypt(`${dto.first_name}`),
                    last_name: await Encrypt(`${dto.last_name}`),
                    middle_name: await Encrypt(`${dto.middle_name}`),
                    maiden_name: await Encrypt(`${dto.maiden_name}`),
                    gender: await Encrypt(`${dto.gender}`)
                })
            })
        }
